...
def pca(X, dims=50, index=0):
    Xn, means = normalize(X)
    Cov = np.matmul(np.transpose(Xn),Xn)

    Xtf = tf.compat.v1.placeholder(tf.float32, shape=[X.shape[0], X.shape[1]])
    Covtf = tf.compat.v1.placeholder(tf.float32, shape=[Cov.shape[0], Cov.shape[1]])
